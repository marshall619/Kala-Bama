...
@Composable
fun AddressSummery(viewModel: BasketViewModel = hiltViewModel(),needToPayMore : Boolean , payablePrice : (Long) -> Unit) {
    val cartDetails by viewModel.cartDetails.collectAsState()
    val sendPrice = if (needToPayMore) digitByLocate(" ${digitBySeparator(30000.toString())}") else "پس کرایه"
    val totalPayable = if (needToPayMore) cartDetails.payablePrice+30000 else  cartDetails.payablePrice
    payablePrice(totalPayable)

    Card(
        modifier = Modifier
            .fillMaxWidth().padding(horizontal = LocalSpacing.current.small),
        elevation = 4.dp,
        ...
        Column(
                    ...
                    if (needToPayMore){
                        Icon(
                            ...
                            modifier = Modifier
                                .size(30.dp)
                                .padding(horizontal = LocalSpacing.current.extraSmall),
                            tint = MaterialTheme.colors.trashColorBasket
                        )
                    }
                }
            }
            ...
            Row(
                ...
                Row(
                    ...
                    Text(
                        text = digitByLocate(" ${digitBySeparator(totalPayable.toString())}"),
                        style = Typography.h4,
                        color = MaterialTheme.colors.mainColor,
                        fontWeight = FontWeight.Bold
                    )
